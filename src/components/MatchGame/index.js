...
class MatchGame extends Component {
  ...
  componentDidMount() {
    this.intervalId = setInterval(this.runSeconds, 1000)
  }
  ...
  checkClickedImg = imageUrl => {
    ...
    if (imageUrl !== mainImgUrl) {
      ...
      this.setState(prevState => ({
        ...
        mainImgUrl: imagesList[randomImageIndex].imageUrl,
      }))
    }
  }
  ...
  renderGame = () => {
    const {mainImgUrl, activeTab} = this.state
    ...
    return (
      <div className="app-container">
        <img src={mainImgUrl} className="main-img" alt="match" />
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <Tabs
              ...
              clickTabToActive={this.clickTabToActive}
            />
          ))}
        </ul>
        <ul className="tab-item-list-container">
          {filteredTabItems.map(eachItem => (
            <TabItem
              key={eachItem.id}
              details={eachItem}
              checkClickedImg={this.checkClickedImg}
            />
          ))}
        </ul>
      </div>
  ...
  resetGame = () => {
    this.setState({gameFinished: false, secondsElapsed: 60, score: 0})
  }
  ...
  render() {
    const {secondsElapsed, score, gameFinished} = this.state
          ...
          <ul className="nav-bar-wrapper">
            ...
            <li className="score-timer-container">
              ...
              <img
                ...
                className="timer"
              />
              <p className="seconds">{secondsElapsed} sec</p>
            </li>
          </ul>
        </nav>
        {gameFinished ? this.renderScoreCard() : this.renderGame()}
      </div>
    )
  }
}
